...
const ObtenerAllEmpAct = async (req, res) => {
    empleados.findAll({where: {Activo: 0}}).then((users) => {
        res.json(users)
    }).catch((err) => {
        console.error(err)
        res.status(500).json({ message: "An error occurred" });
...
const ObtenerAllEmpInac = async (req, res) => {
    empleados.findAll({where: {Activo: 1}}).then((users) => {
        res.json(users)
    }).catch((err) => {
        console.error(err)
        res.status(500).json({ message: "An error occurred" });
...
const UpdateEmp = async (req, res) => {
    ...
    const updateEmp = {
        ...
        Edad: req.body.Edad || null,
        A_servicio: req.body.Aservicio || null,
        FechaRetiro: req.body.FechaR,
        CausaRetiro: req.body.CausaR
    }
    console.log(req.body.FechaR)
...
module.exports = { ObtenerAllEmpAct, CrearEmp, UpdateEmp, DeleteEmp, DatosEmp, ActiveEmp, InfoEmp, ObtenerAllEmpInac, ObtenerAllEmp }